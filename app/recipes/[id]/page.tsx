            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            <Heart
                                className={`h-4 w-4 transition-colors duration-300 ${isFavorite(recipe.id) ? 'fill-red-500' : ''}`}/>
                        </Button>
                        <Link href={`/recipes/${recipe.id}/cook`}>
                            <Button
                                className="bg-green-600 text-white hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-600">Gotuj
                                przepis</Button>
                        </Link>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    <Badge
                        className="bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100">{recipe.type}</Badge>
                    ...
                    >
                        ...
                        <span
                            className={`transition-transform duration-300 ${isFavorite(recipe.id) ? 'scale-110' : 'scale-100'}`}>
                        ...
                        <CardContent className="space-y-4">
                                ...
                                <Card key={`${comment.id}-${index}`} className="bg-gray-50 dark:bg-gray-800">
                                    <CardContent className="p-4">
                                        <div className="flex items-center justify-between gap-2 mb-2">
                                            ...
                                            <div
                                                className="text-sm text-gray-600 dark:text-gray-400">{new Date(comment.date).toLocaleString()}</div>
                                        </div>
                                        {editingComment === comment.id ? (
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex justify-end gap-2">
                                                    <Button onClick={() => setEditingComment(null)} variant="outline"
                                                            size="sm">Anuluj</Button>
                                            ...
                                            <div className="flex justify-between items-start">
                                                <p>{comment.content}</p>
                                                <div className="flex gap-2">
                                                    <Button onClick={() => startEditing(comment)} variant="ghost"
                                                            size="sm">
                                                        <Edit className="h-4 w-4"/>
                                                    </Button>
                                                    <Button onClick={() => deleteComment(comment.id)} variant="ghost"
                                                            size="sm">
                            ...
                            <div className="mt-4 space-y-2">
                                ...
                                <Button onClick={addComment}
                                        className="bg-green-500 text-white hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700 transition-colors">Dodaj
                                    komentarz</Button>
                            </div>
                        </CardContent>
                    </Card>
